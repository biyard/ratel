...
class BoardsCreatorController extends BaseController {
  ...
  void onInit() {
    ...
    if (rawPk == null || rawPk.isEmpty) {
      logger.w(
        'BoardsCreatorController: spacePk is null or empty in parameters',
      );
      return;
    }

    spacePk = Uri.decodeComponent(rawPk);
    logger.d('BoardsCreatorController initialized with spacePk=$spacePk');
  ...
  Future<void> loadCategories() async {
    try {
      ...
      logger.d('BoardsCreator categories loaded: $categories');
    } catch (e) {
  ...
  Future<void> loadPosts({bool reset = false}) async {
    ...
    try {
      ...
      logger.d(
        'Loaded posts for spacePk=$spacePk, count=${res.posts.length}, bookmark=$_bookmark',
      );

      logger.d('BoardsCreator posts list (total=${posts.length}):');
      for (var i = 0; i < posts.length; i++) {
        logger.d('  [$i] ${posts[i]}');
      }
    } catch (e) {
      logger.e('Failed to load posts for spacePk=$spacePk: $e');
      Biyard.error(
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      logger.d(
        'Loaded more posts for spacePk=$spacePk, added=${res.posts.length}, bookmark=$_bookmark',
      );

      logger.d(
        'BoardsCreator posts list after loadMore (total=${posts.length}):',
      );
      for (var i = 0; i < posts.length; i++) {
        logger.d('  [$i] ${posts[i]}');
      }
    } catch (e) {
      logger.e('Failed to load more posts for spacePk=$spacePk: $e');
    } finally {
