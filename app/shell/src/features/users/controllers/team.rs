...
pub async fn create_team_handler(body: CreateTeamRequest) -> crate::features::users::Result<CreateTeamResponse> {
    let Extension(session) = session;
    let user_pk: String = session
        .get::<String>("user_id")
        .await
        .map_err(|e| crate::features::users::Error::Unauthorized(e.to_string()))?
    ...
    {
        return Err(crate::features::users::Error::BadRequest(
