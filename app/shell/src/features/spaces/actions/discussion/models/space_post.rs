...
use crate::features::spaces::actions::discussion::macros::DynamoEntity;
use crate::features::spaces::actions::discussion::models::{SpacePostComment, SpacePostCommentLike};
...
impl SpacePost {
    ...
    pub async fn like_comment(
    ...
    ) -> crate::features::spaces::actions::discussion::Result<()> {
        ...
        let comment = SpacePostComment::get(cli, &space_post_pk_p, Some(comment_pk.clone()))
            .await?
            .ok_or(crate::features::spaces::actions::discussion::Error::NotFound("Comment not found".into()))?;
...
impl SpacePost {
    ...
    pub fn can_view(_user_role: &SpaceUserRole) -> crate::features::spaces::actions::discussion::Result<()> {
        Ok(())
    }

    pub fn can_edit(user_role: &SpaceUserRole) -> crate::features::spaces::actions::discussion::Result<()> {
        match user_role {
            SpaceUserRole::Creator => Ok(()),
            _ => Err(crate::features::spaces::actions::discussion::Error::NoPermission),
        }
    }

    pub fn can_participate(&self, user_role: &SpaceUserRole) -> crate::features::spaces::actions::discussion::Result<()> {
        match user_role {
            SpaceUserRole::Creator => Ok(()),
            SpaceUserRole::Participant => {
                if self.status() == DiscussionStatus::InProgress {
                    ...
                    Err(crate::features::spaces::actions::discussion::Error::SpacePostNotStarted)
                }
            }
            _ => Err(crate::features::spaces::actions::discussion::Error::NoPermission),
        }
