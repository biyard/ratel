...
use crate::features::spaces::actions::poll::macros::DynamoEntity;
...
#[cfg(feature = "server")]
impl SpacePoll {
    pub fn new(space_pk: SpacePartition) -> crate::features::spaces::actions::poll::Result<Self> {
    ...
    pub async fn delete_one(
        ...
        space_pk: &Partition,
    ) -> crate::features::spaces::actions::poll::Result<()> {
...
impl SpacePoll {
    pub fn can_view(_user_role: &SpaceUserRole) -> crate::features::spaces::actions::poll::Result<()> {
        Ok(())
    }
    ...
    pub fn can_edit(user_role: &SpaceUserRole) -> crate::features::spaces::actions::poll::Result<()> {
        match user_role {
            SpaceUserRole::Creator => Ok(()),
            _ => Err(crate::features::spaces::actions::poll::Error::NoPermission),
        }
    }

    pub fn can_respond(&self, user_role: &SpaceUserRole) -> crate::features::spaces::actions::poll::Result<()> {
        match user_role {
...
impl TryFrom<Partition> for SpacePoll {
    type Error = crate::features::spaces::actions::poll::Error;

    fn try_from(value: Partition) -> crate::features::spaces::actions::poll::Result<Self> {
        let uuid = match value {
            Partition::Space(ref s) => s.clone(),
            _ => {
                return Err(crate::features::spaces::actions::poll::Error::InternalServerError(
