...
impl Partition {
    pub fn to_space_pk(self) -> crate::common::Result<Partition> {
        match self {
            Partition::Feed(pk) => Ok(Partition::Space(pk)),
            _ => Err(crate::common::Error::InvalidPartitionKey(
                "Space key can be only extracted from Feed key".to_string(),
            )),
        }
    }

    pub fn to_post_key(self) -> crate::common::Result<Partition> {
        match self {
    ...
    pub fn to_post_like_key(self) -> crate::common::Result<Partition> {
        match self {
    ...
    pub fn to_poll_sk(&self) -> crate::common::Result<EntityType> {
        match self {
