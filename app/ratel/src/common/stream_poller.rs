    ...
    tokio::spawn(async move {
        ...
        let stream_arn = match get_stream_arn(dynamodb, &table_name).await {
            ...
            None => {
                tracing::warn!(
                    table = %table_name,
                    "No DynamoDB stream found, stream poller disabled"
                );
                return;
            }
        };
            ...
            for (shard_id, iterator) in &shard_iterators {
                    ...
                    .await
                {
                    Ok(output) => {
                        ...
                        // If no next iterator, shard is exhausted — don't re-add
                    }
                    Err(e) => {
                        tracing::warn!(
                            shard_id = %shard_id,
                            error = %e,
                            "Failed to get records, will refresh shard iterator"
                        );
                        // Drop the stale iterator; it'll be re-acquired next cycle
                    }
                }
            }
