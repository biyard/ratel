...
pub fn ActionSettingsModal(
    ...
    rsx! {
        div { class: "flex justify-end w-full h-full text-web-font-primary",
            div { class: "flex flex-col gap-5 py-6 px-6 w-full h-full shrink-0 overflow-y-auto bg-web-card-bg max-tablet:px-5 max-tablet:py-5 max-mobile:min-h-full max-mobile:gap-4 max-mobile:px-4 max-mobile:py-4",
