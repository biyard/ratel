...
pub async fn create_discussion(space_id: SpacePartition) -> Result<SpacePost> {
    ...
    let mut items = vec![post.create_transact_write_item(), space_action.create_transact_write_item()];
    ...
    crate::transact_write_items!(cli, items)
        .map_err(|e| crate::features::spaces::pages::actions::actions::discussion::Error::Unknown(format!("Failed to create discussion: {e}")))?;
