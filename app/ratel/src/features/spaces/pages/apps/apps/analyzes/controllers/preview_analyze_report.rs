...
/// detail page uses the same matching logic against the saved filters
/// once the stream-based analysis pipeline lands.
#[post(
...
pub async fn preview_analyze_report(
    ...
    if req.filters.is_empty() {
        let participants = count_all_participants(cli, &space_pk).await?;
    ...
    for filter in &req.filters {
        let (matched, records) = match_filter(cli, &space_id, &space_pk, filter).await?;
        data_count += records;
    ...
    Ok(PreviewAnalyzeReportResponse {
        respondent_count: intersection.unwrap_or_default().len() as i64,
        data_count,
    })
}

#[cfg(feature = "server")]
async fn list_participant_user_pks(
...
#[cfg(feature = "server")]
async fn count_all_participants(
    cli: &aws_sdk_dynamodb::Client,
    space_pk: &Partition,
) -> Result<i64> {
