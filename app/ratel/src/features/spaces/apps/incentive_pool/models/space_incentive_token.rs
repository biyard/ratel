...
impl SpaceIncentiveToken {
    ...
    ) -> crate::features::spaces::apps::incentive_pool::Result<(Vec<Self>, Option<String>)> {
    ...
    pub async fn list_token_addresses(
        cli: &aws_sdk_dynamodb::Client,
        incentive_address: impl std::fmt::Display,
    ) -> crate::features::spaces::apps::incentive_pool::Result<std::collections::HashSet<String>> {
        let (items, _) =
    ...
    pub async fn upsert_balance(
        ...
        balance: String,
        updated_at: i64,
    ) -> crate::features::spaces::apps::incentive_pool::Result<()> {
        let item = Self::new(
            incentive_address,
            token_address,
            symbol,
