...
use crate::features::teams::pages::setting::i18n::TeamSettingsTranslate;
...
#[component]
pub fn TeamSettingLayout(teamname: String) -> Element {
    ...
    rsx! {
        div { class: "grid overflow-hidden grid-cols-1 w-full h-screen tablet:grid-cols-[250px_1fr] bg-team-bg text-text-primary",
            div { class: "hidden tablet:flex",
                SettingsSidemenu { teamname: teamname.clone() }
            }
...
#[component]
fn SettingsSidemenu(teamname: String) -> Element {
    let user_ctx = crate::features::auth::hooks::use_user_context();
    ...
    let team_item = {
        ...
        teams.iter().find(|t| t.username == teamname).cloned()
    };
    ...
    let (team_display, team_profile, permissions_val) = match &team_item {
        ...
        None => (teamname.clone(), String::new(), 0i64),
    };
    ...
    let team_home_route = Route::TeamHome { teamname: teamname.clone() }.to_string();
    let settings_route = Route::TeamSetting { teamname: teamname.clone() }.to_string();
    let management_route = Route::TeamSettingMember { teamname: teamname.clone() }.to_string();
