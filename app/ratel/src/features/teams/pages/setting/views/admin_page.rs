...
pub fn AdminPage(teamname: String, team: TeamResponse) -> Element {
    ...
    // Pre-clone teamname for use_effect so the original remains available for on_open_delete
    let teamname_for_save = teamname.clone();
    ...
    use_effect(move || {
        ...
        let teamname = teamname_for_save.clone();
