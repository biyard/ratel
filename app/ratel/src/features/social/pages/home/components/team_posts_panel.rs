...
fn TeamPostCard(post: PostResponse, #[props(default = false)] full_width: bool) -> Element {
    ...
    rsx! {
        div {
            ...
            div { class: "flex h-full flex-col gap-4 border-b border-separator py-6",
                    ...
                    if !post_categories.is_empty() {
                        div { class: "flex min-w-0 items-center gap-2 flex-wrap",
                            for cat in post_categories.iter() {
                                div {
                                    key: "{cat}",
                                    class: "flex h-[25px] items-center justify-center rounded-[8px] border border-border px-2 py-[3px]",
                                    span { class: "font-bold text-[12px]/[14px] tracking-[-0.12px] text-text-primary",
                                        "{cat}"
                                    }
                                }
                            }
                ...
                h2 { class: "font-bold text-[20px]/[25px] tracking-[0.5px] text-text-primary line-clamp-1",
                    "{title}"
                }

                div { class: "flex items-center justify-between",
                    span { class: "text-[15px]/[22px] text-foreground-muted",
                    ...
                    div { class: "flex items-center gap-[10px]",
                        ...
                        div { class: "flex items-center gap-[5px]",
                            ...
                            span { class: "text-[15px]/[18px] font-medium text-text-primary",
                                "{comments}"
                            }
                        }
                    }
...
fn TeamPostListItem(post: PostResponse) -> Element {
    ...
    rsx! {
        div {
            ...
            div { class: "flex w-full flex-col border-b border-separator py-6",
                ...
                h2 { class: "font-bold text-[26px]/[30px] text-text-primary line-clamp-2 mb-2.5",
                    "{title}"
                }
