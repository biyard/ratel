...
pub fn PointsBarChart(
    ...
    rsx! {
        div { class: "py-10 border-t border-card-border",
            div { class: "flex flex-col gap-3 items-center w-full",
                // Period selector tabs
                div { class: "relative w-full",
                    div { class: "flex items-center border border-white/10 rounded-[10px] h-[39px]",
                        button {
                            class: format!(
                ...
                // Date range label
                span { class: "font-semibold text-[15px] text-text-primary tracking-[-0.075px]",
                    "{date_range}"
                }

                // Chart area
                div { class: "flex items-end mt-2 w-full gap-[45px]",
                    // Y-axis labels
                    div { class: "flex flex-col gap-12 items-end w-20 text-sm font-normal opacity-80 text-foreground-muted shrink-0",
                        span { "100%" }
                        span { "60%" }
                        span { "20%" }
                    }
                ...
                // X-axis labels
                div { class: "flex justify-between items-end pr-4 w-full text-sm font-normal opacity-80 pl-[125px] text-foreground-muted",
                    for bar in bars.iter() {
...
fn render_bar(bar: &ChartBar, max_points: i64) -> Element {
    ...
    rsx! {
        div {
            class: "w-20 bg-gradient-to-b from-[var(--web\\/graph\\/bar,#fcb300)] to-[var(--web\\/graph\\/bar2,#1a1a1a)] rounded-t-[10px]",
