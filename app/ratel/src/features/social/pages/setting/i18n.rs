use super::*;

translate! {
    TeamSettingsTranslate;

    username: {
        en: "Username",
        ko: "사용자명",
    },

    display_name: {
        en: "Display Name",
        ko: "닉네임",
    },

    description: {
        en: "Description",
        ko: "설명",
    },

    dao_address: {
        en: "DAO Address",
        ko: "DAO 주소",
    },

    upload_logo: {
        en: "Upload Logo",
        ko: "로고 업로드",
    },

    display_name_hint: {
        en: "Enter team display name",
        ko: "팀 닉네임을 입력하세요",
    },

    team_description_hint: {
        en: "Enter team description",
        ko: "팀 설명을 입력하세요",
    },

    activate_dao: {
        en: "Activate DAO",
        ko: "DAO 활성화",
    },

    activating_dao: {
        en: "Activating...",
        ko: "활성화 중...",
    },

    edit: {
        en: "Edit",
        ko: "수정",
    },

    save: {
        en: "Save",
        ko: "저장",
    },

    save_changes: {
        en: "Save Changes",
        ko: "변경사항 저장",
    },

    delete: {
        en: "Delete",
        ko: "삭제",
    },

    cancel: {
        en: "Cancel",
        ko: "취소",
    },

    confirm: {
        en: "Confirm",
        ko: "확인",
    },

    success_delete_team: {
        en: "Team deleted successfully",
        ko: "팀이 성공적으로 삭제되었습니다",
    },

    failed_delete_team: {
        en: "Failed to delete team",
        ko: "팀 삭제에 실패했습니다",
    },

    success_update_team: {
        en: "Team settings saved successfully",
        ko: "팀 설정이 저장되었습니다",
    },

    failed_update_team: {
        en: "Failed to update team",
        ko: "팀 업데이트에 실패했습니다",
    },

    remove_test_keyword: {
        en: "Please remove test keywords",
        ko: "테스트 키워드를 제거해주세요",
    },

    delete_team_title: {
        en: "Delete Team",
        ko: "팀 삭제",
    },

    delete_team_description: {
        en: "Are you sure you want to delete this team? This action cannot be undone.",
        ko: "정말로 이 팀을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.",
    },

    validation_nickname_required: {
        en: "Display name is required",
        ko: "닉네임은 필수입니다",
    },

    validation_description_min_length: {
        en: "Description must be at least 10 characters",
        ko: "설명은 최소 10자 이상이어야 합니다",
    },

    general_settings: {
        en: "General settings",
        ko: "일반 설정",
    },

    thumbnail: {
        en: "Thumbnail",
        ko: "썸네일",
    },

    team_logo: {
        en: "Team Logo",
        ko: "팀 로고",
    },

    team_name: {
        en: "Team name",
        ko: "팀 이름",
    },

    change_name: {
        en: "Change name",
        ko: "이름 변경",
    },

    last_saved_at: {
        en: "Last saved at",
        ko: "마지막 저장",
    },

    upload_banner: {
        en: "Upload banner image",
        ko: "배너 이미지 업로드",
    },

    thumbnail_hint: {
        en: "Recommended size: 1200 × 400px (16:9 ratio) • Max file size: 5MB",
        ko: "권장 크기: 1200 × 400px (16:9 비율) • 최대 파일 크기: 5MB",
    },

    team_logo_hint: {
        en: "Recommended size: 256 × 256px (1:1 ratio) • Max file size: 2MB",
        ko: "권장 크기: 256 × 256px (1:1 비율) • 최대 파일 크기: 2MB",
    },

    delete_team: {
        en: "Delete team",
        ko: "팀 삭제",
    },

    back_to_page: {
        en: "Back to page",
        ko: "페이지로 돌아가기",
    },

    settings: {
        en: "Settings",
        ko: "설정",
    },

    no_permission_title: {
        en: "No permission",
        ko: "권한이 없습니다",
    },

    no_permission_description: {
        en: "You don't have permission to view this page.",
        ko: "이 페이지에 접근 권한이 없습니다.",
    },

    team_management: {
        en: "Team management",
        ko: "팀 관리",
    },

    add_members: {
        en: "Add members",
        ko: "멤버 추가",
    },

    members: {
        en: "Members",
        ko: "멤버",
    },

    no_members: {
        en: "No members found.",
        ko: "멤버가 없습니다.",
    },

    remove_from_team: {
        en: "Remove from team",
        ko: "팀에서 제거",
    },

    owner: {
        en: "Owner",
        ko: "소유자",
    },

    member_role: {
        en: "Member",
        ko: "멤버",
    },

    admin_role: {
        en: "Admin",
        ko: "관리자",
    },

    failed_remove_member: {
        en: "Failed to remove member. Please try again.",
        ko: "멤버 제거에 실패했습니다. 다시 시도해주세요.",
    },

    subscription: {
        en: "Subscription",
        ko: "구독",
    },
}
