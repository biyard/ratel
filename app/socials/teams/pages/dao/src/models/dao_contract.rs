...
pub async fn create_dao(
    ...
    let promise = crate::interop::create_dao(&admins_js, network, rpc_url, block_explorer_url)
...
#[cfg(not(feature = "server"))]
fn format_js_error(err: wasm_bindgen::JsValue) -> DaoWalletError {
    use web_sys::js_sys::{JSON, Reflect};

    let code = if err.is_object() {
        Reflect::get(&err, &wasm_bindgen::JsValue::from_str("code"))
            .ok()
    ...
    let message = if let Some(msg) = err.as_string() {
        ...
        Reflect::get(&err, &wasm_bindgen::JsValue::from_str("message"))
            .ok()
            .and_then(|value| value.as_string())
