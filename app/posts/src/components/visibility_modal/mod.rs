...
#[component]
pub fn VisibilityModal(
    #[props(default = Visibility::Public)] initial: Visibility,
    on_confirm: EventHandler<Visibility>,
    on_cancel: EventHandler<MouseEvent>,
) -> Element {
    let tr: VisibilityModalTranslate = use_translate();
    let mut selected = use_signal(move || initial.clone());

    let is_public = matches!(*selected.read(), Visibility::Public);
