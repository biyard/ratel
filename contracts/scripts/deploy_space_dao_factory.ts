import { ethers, network } from "hardhat";


async function main() {
  console.log(`Deploying SpaceDAO factory on network: ${network.name}`);
  console.log("Deployer:", (await ethers.getSigners())[0].address);

  const rewardExtensionAddr = process.env.REWARD_EXTENSION_ADDR;
  const spaceDaoAddr = process.env.SPACE_DAO_ADDR;
  
  if (!spaceDaoAddr || !rewardExtensionAddr) {
    throw new Error("Logic addresses are missing in the deployment file.");
  }

  console.log("SpaceDAO Address:", spaceDaoAddr);
  console.log("RewardExtension Address:", rewardExtensionAddr);

  console.log("Deploying SpaceFactory...");
  const SpaceFactory = await ethers.getContractFactory("SpaceFactory");
  
  const factory = await SpaceFactory.deploy(spaceDaoAddr, rewardExtensionAddr);
  await factory.waitForDeployment();
  const factoryAddr = await factory.getAddress();

  console.log("SpaceFactory deployed at:", factoryAddr);

}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});