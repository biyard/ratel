...
async function main() {
  ...
  const teamDaoLogic = await TeamDAO.deploy();
  await teamDaoLogic.waitForDeployment();
  const teamDaoAddr = await teamDaoLogic.getAddress();
  console.log("SpaceDAO deployed at:", teamDaoAddr);
  ...
  console.log("SpaceDAO Address:", teamDaoAddr);
