...
test.describe.serial("Space with lots actions", () => {
  ...
  async function createSpaceFromPost(page) {
    // The post-edit renewal removed the "skip-space-checkbox" and
    // "Go to Space" affordances. Create post + space via REST endpoints so
    // this suite can focus on action creation inside the space.
    const postRes = await page.request.post("/api/posts", { data: {} });
    if (!postRes.ok()) {
      throw new Error(`create post failed: ${await postRes.text()}`);
    }
    ...
    if (!spaceRes.ok()) {
      throw new Error(`create space failed: ${await spaceRes.text()}`);
    }
  ...
  test("Admin adds a Meet via TypePicker", async ({ page }) => {
    ...
    // Capture the meet action URL so subsequent tests can re-navigate to it
    // — each Playwright test block gets a fresh Page, so URL state doesn't
    // carry over even inside test.describe.serial.
    meetUrl = new URL(page.url()).pathname;
  });

  test("Admin toggles mode Scheduled ↔ Instant", async ({ page }) => {
    await goto(page, meetUrl);
