...
async fn main() -> Result<()> {
    ...
    for user in generate_test_users(no_users, test_user_password) {
        ...
        if is_created {
            if let Err(e) = api_client
                ...
                .await
            {
                println!("✗ Failed to add oracle for user {}: {}", user.nickname, e);
            }
        } else {
            if let Err(e) = api_client
    ...
    //Save Space ID to spaces.csv
    {
        ...
        let mut file = BufWriter::new(File::create("spaces.csv")?);
    ...
    // Save user Tokens to tokens.csv
    {
        ...
        let mut file = BufWriter::new(File::create("tokens.csv")?);
    ...
    //Save artwork IDs to artworks.csv
    {
        ...
        let mut file = BufWriter::new(File::create("artworks.csv")?);
