...
const buildPdfBlob = async (htmlDocument: string): Promise<Blob> => {
...
const uploadPdf = async (presignedUrl: string, blob: Blob) => {
...
>() {
  ...
  return useMutation({
    mutationFn: async ({ spacePk }: { spacePk: string }) => {
      ...
      const pdfBlob = await buildPdfBlob(res.html_document);
      await uploadPdf(res.presigned_url, pdfBlob);
