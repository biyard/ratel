...
export default function RemoteGalleryView({
  ...
  const selfUserId = selfUser?.user_pk;
  ...
  const sortedParticipants = useMemo(() => {
    ...
    const self = participants.find((p) => p.user_pk === selfUserId);
    const others = participants.filter((p) => p.user_pk !== selfUserId);
      ...
      >
        ...
        >
          {sortedParticipants.map((p) => {
            const user = users.find((u) => u.user_pk === p.user_pk);
            ...
            const nickname = p.author_username;
