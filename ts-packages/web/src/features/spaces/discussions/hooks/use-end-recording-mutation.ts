...
import { discussionEndRecording } from '@/lib/api/ratel/discussion.spaces.v3';

export function useEndRecordingMutation<T extends SpaceDiscussionResponse>() {
  const mutation = useMutation({
    mutationKey: ['end-recording'],
    mutationFn: async ({
      spacePk,
      discussionPk,
    }: {
      ...
      await discussionEndRecording(spacePk, discussionPk);
    },
