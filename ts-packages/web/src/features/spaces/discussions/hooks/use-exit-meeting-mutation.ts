...
import { discussionExitMeeting } from '@/lib/api/ratel/discussion.spaces.v3';

export function useExitMeetingMutation<T extends SpaceDiscussionResponse>() {
  const mutation = useMutation({
    mutationKey: ['exit-meeting'],
    mutationFn: async ({
      spacePk,
      discussionPk,
    }: {
      ...
      await discussionExitMeeting(spacePk, discussionPk);
    },
