...
export default function TimelineMenu({
  ...
  return (
    <div className="flex flex-col w-full text-text-secondary gap-5 px-1 py-3">
      <div className="flex flex-row font-bold text-sm justify-between">
        <div className="flex flex-row gap-2">
          <Clock className="size-5 *:stroke-text-secondary" />
          {titleLabel}
        </div>
        {isEditing && (
          <button
          ...
          >
            <Settings2 className="size-5 *:stroke-text-secondary" />
          </button>
        )}
      </div>
      <div className="flex flex-col pl-3.25 gap-5">
        {sortedItems.map((item) => (
          ...
          >
            ...
            <div className="font-medium text-text-secondary text-xs/[12px]">
