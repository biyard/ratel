...
export interface ApiCallFns {
  get: (path: string) => Promise<any>;
  post: (path: string, body?: any) => Promise<any>;
  del: (path: string, body?: any) => Promise<any>; //New delete method
}
...
export function useApiCall(): ApiCallFns {
  ...
  async function prepareAuthHeaders(): Promise<{ [key: string]: string }> {
    let token = cookie?.token;
    let token_type = 'Bearer';
  ...
  return {
    ...
    del: async (path: string, body?: any): Promise<any> => {
      const headers = await prepareAuthHeaders();
      const response = await fetch(`${config.api_url}${path}`, {
        method: 'DELETE',
        headers,
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({
          message: 'Failed to delete and parse error',
        }));
        logger.error('DELETE error:', errorData?.message);
        return null;
      }
