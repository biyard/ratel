...
팀 설정(Team Settings) 은 Team 의 관리자가 Team 자체를 관리하는 곳입니다 — 공개 정체성, 멤버와 역할, 매월 Credit 을 지급하는 구독, 그리고 (필요하다면) 삭제 흐름까지. 이 화면은 Team 의 **관리자(administrator) 시점** 입니다 — 공개 프로필과 피드는 [팀 만들기](./teams) 챕터를 참고하세요.
...
- **DAO Address** *(DAO 활성화 시)* — 이 Team 에 연결된 온체인 트레저리 / 거버넌스 주소. 여기서는 읽기 전용이고, DAO 활성화는 [팀 → DAO](./teams#dao--collective-governance) 흐름에서 이뤄집니다.
...
Team 구독은 사용자 구독과 동일한 다섯 등급을 사용하며, Credit 할당량과 스페이스당 한도도 같습니다. 전체 기능 매트릭스는 [멤버십](./membership) 챕터를 참고하세요.
...
- **하위팀** (신청, 관리, 등록 해제, 이탈) — [하위팀](./sub-teams) 참고. 거버넌스 흐름이지 계정 설정이 아닙니다.
...
- [팀 만들기](./teams) — 멤버, 초안, DAO 기초를 다루는 기본 Team 챕터.
- [하위팀](./sub-teams) — 상위 / 하위 Team 관계의 전체 거버넌스 라이프사이클.
- [멤버십](./membership) — Team 의 보상 스페이스를 결제할 월간 Credit 등급을 선택하세요.
