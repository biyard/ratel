...
- **스페이스 게이팅** — 호스트가 인증된 속성 (예: *나이 ≥ 19*, *`@university.alpha` 멤버*) 을 참여 또는 보상 청구의 조건으로 걸 수 있습니다. 호스트는 스페이스의 [Panels 앱](./spaces/apps#-panels) 또는 개별 액션 설정에서 이를 구성합니다.
- **온체인 동작** — 본인의 DID 가 [보상 청구 흐름](./rewards#%EC%98%A8%EC%B2%B4%EC%9D%B8-%EC%A0%95%EC%82%B0) 의 서명 기준입니다. 이 암호학적 신원이 오프 플랫폼체인 포인트 장부와 온체인 청구 트랜잭션을 연결합니다.
...
- **지갑 불필요** — DID 는 Ratel 계정에 묶여 있습니다. DID 를 *가지는* 데는 블록체인 지갑이 필요 없고, 거기에 연결된 *온체인 보상을 청구하는* 데에만 지갑이 필요합니다 ([리워드](./rewards) 참고).
...
- [설정](./settings) — DID 가 묶인 기반 계정을 관리합니다.
- [리워드](./rewards) — 본인의 DID 가 온체인 청구 트랜잭션 서명에 어디서 사용되는지 확인합니다.
- [스페이스 → 앱 → Panels](./spaces/apps#-panels) — 호스트 측에서 크리덴셜로 스페이스 접근을 게이팅하는 화면.
