...
The subscription unlocks team-level features (monthly Credits for reward Spaces, the Trusted Creator badge, raw participant data access at higher tiers). Billing is handled off-chain via PortOne, so you can pay with the methods you already use. See [Team Settings → Team subscription](./team-settings#-team-subscription) for the full breakdown.
