...
impl CharacterSkill {
    ...
    /// Read a skill row's level, treating "row absent" as level 0.
...
pub async fn get_character_handler() -> Result<CharacterResponse> {
    #[cfg(feature = "server")]
    {
        ...
        let xp = CharacterXp::get(cli, &xp_pk, Some(&xp_sk))
            .await?
            .unwrap_or_else(|| CharacterXp::new(user.pk.clone()));

        let mut skills = Vec::with_capacity(2);
        for id in [SkillId::MoneyTree, SkillId::Ranker, SkillId::Influencer, SkillId::Sweeper] {
            let lv = CharacterSkill::level_or_zero(cli, &user.pk, id).await?;
            skills.push((id, lv));
        }
...
pub async fn level_up_handler(skill_id: String) -> Result<CharacterResponse> {
    #[cfg(feature = "server")]
    {
        ...
        // Re-read assembled state for response.
        let xp = CharacterXp::get(cli, &xp_pk, Some(&xp_sk))
            .await?
            .ok_or(Error::NotFound)?;
        let mut skills = Vec::with_capacity(4);
        for sid in [SkillId::MoneyTree, SkillId::Ranker, SkillId::Influencer, SkillId::Sweeper] {
            skills.push((sid, CharacterSkill::level_or_zero(cli, &user.pk, sid).await?));
        }
...
pub async fn run(cli: &aws_sdk_dynamodb::Client) -> crate::common::Result<()> {
    ...
    // Upsert CharacterXp per user.
    for (user_pk, total_xp) in totals {
        ...
        // Upsert: create if not exists; if exists, refresh fields. Use a put.
        row.put(cli).await?;
    }

    for (user_pk, space_id, last_seen) in sources {
        let now = crate::common::utils::time::get_now_timestamp_millis();
