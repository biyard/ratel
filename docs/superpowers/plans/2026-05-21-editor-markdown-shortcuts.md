...
**Goal:** Add Notion-style markdown shortcuts to the custom contentEditable editor so that typing `# `, `## `, `### `, `* `/`- `/`+ `, `1. `, `> `, ` ``` `, `--- `, `*** ` at the start of a block converts the block to the matching HTML structure; `Tab`/`Shift+Tab` nest/un-nest list items; `Enter` on an empty `<li>` exits the list; and one-step `Backspace` reverts a just-fired conversion. (Spec §4 also documented `Cmd+Z`/`Ctrl+Z` revert and `selectionchange`-based disarm — both are explicitly deferred to a follow-up and not part of this delivery.)

**Architecture:** JS-only change. All logic appended to the existing per-editor `init(root)` closure in `app/ratel/src/common/components/editor/script.js` so it shares the `composing`, `editor`, `scheduleUpdate`, `applyCmd` bindings. No Rust changes. Conversions are implemented via **direct DOM manipulation** rather than `document.execCommand` — Chrome's contentEditable engine silently no-ops nested `execCommand("formatBlock", ...)` calls when invoked from inside the input handler of a triggering `execCommand("insertText", ...)`, and `execCommand("indent")` produces invalid list nesting (sibling `<ul>` instead of child of `<li>`). Backspace revert is implemented as a snapshot of `editor.innerHTML` with a `﻿`-wrapped sentinel marking caret position, restored on revert.
