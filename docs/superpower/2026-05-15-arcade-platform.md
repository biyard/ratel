...
| **A2'** | **v1 transport = polling (채팅도 포함). v2 = WebSocket** | v1: 모든 라운드 상태가 클라 2~3초 폴링 — 라운드 메타는 `GET /rounds/{id}`, 채팅은 `GET /rounds/{id}/chat?since=<last_msg_id>`. **SSE 인프라 (PR4e/PR4f + CDK Function URL) 는 코드 + 인프라 다 깔려있지만 v1엔 비활성** — Lambda binary가 streaming response 안 emit해서 실시간 SSE가 buffered가 됨 (실효 X). v2: Lambda 코드를 streaming runtime으로 바꾸거나, WebSocket으로 통합. `RoomChannel` trait + `InProcessChannel` hub + DDB Stream listener bridge는 그대로 살아있어 v2엔 publish가 그대로 닿음 |
...
# Realtime (v1: polling 전반)
# 라운드 상태는 클라가 GET /rounds/{id} 매 2~3초 폴링.
GET    /api/arcade/games/fact-or-fold/rounds/{id}/chat?since 채팅 폴링 (since=last_msg_id로 incremental)
POST   /api/arcade/games/fact-or-fold/rounds/{id}/chat       채팅 발화 (HTTP POST, DB write)
GET    /api/arcade/events?channel=fof.chat:{round_id}        SSE 구독 (인프라 살아있음 / v2 활성화)
```

v1 채팅 흐름:
1. 클라가 `POST /chat` — 서버가 `FactFoldChatMessage` row를 DDB에 write
2. 다른 참가자 클라가 매 2~3초 `GET .../chat?since=<last_msg_id>` 호출
3. 서버는 새 메시지만 응답, last_id 갱신
4. Latency 1~3초 (acceptable for v1 베타)

v1엔 SSE 활성화 안 함:
- Lambda binary가 `lambda_http::Adapter` (buffered) 사용 → Function URL이 RESPONSE_STREAM 모드라도 실시간 stream X
- SSE 코드 + DDB Stream listener bridge는 살아있음. v2에 Lambda runtime을 streaming으로 갈아끼우면 그대로 동작
- 또는 v2에서 WebSocket으로 통합
