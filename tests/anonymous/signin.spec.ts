...
test.describe("User Sign-in Flow", () => {
  ...
  test("[SI-001] should reject invalid email format", async ({ page }) => {
    ...
    await page.screenshot({ path: "test-results/SI-001/02-invalid-email-filled.png" });

    await page.getByRole("button", { name: /^continue$/i }).click();
    await page.screenshot({ path: "test-results/SI-001/03-after-continue.png" });
  ...
  test("[SI-002] should reject unregistered email", async ({ page }) => {
    ...
    await page.screenshot({ path: "test-results/SI-002/02-unregistered-email.png" });

    await page.getByRole("button", { name: /^continue$/i }).click();
    await page.screenshot({ path: "test-results/SI-002/03-after-continue.png" });
