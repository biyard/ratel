...
test("[Home page] Testing the Ratel White paper link in a PDF DOC", async ({
  ...
  try {
    ...
    const linkSelector = p
      .locator('a[href*="Ratel-Token-White-Paper.pdf"]')
      .filter({ hasText: "LEARN MORE ABOUT $RATEL" });

    const count = await linkSelector.count();

    if (count > 0) {
      await expect(linkSelector).toBeVisible();
      await expect(linkSelector).toHaveAttribute(
        "href",
        /Ratel-Token-White-Paper\.pdf$/,
      );
