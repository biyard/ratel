...
* **Do not use `Error::BadRequest(String)` for domain-specific errors.** Instead, define a specific error enum with `Translate` derive (e.g., `SpaceActionQuizError`, `SpaceRewardError`) for user-friendly, i18n-compatible error handling. See the `SpaceRewardError` pattern in `app/ratel/src/common/types/reward/error.rs` and the "Error Handling Convention" section of `CLAUDE.md`. When reviewing code that introduces new `Error::BadRequest(...)` calls, flag it and suggest a typed error variant instead. **Important:** `common::Error` uses a manual `Translate` impl (not `derive(Translate)`) that delegates to the inner error's `translate()` for wrapper variants like `SpaceReward(e)` and `SpaceActionQuiz(e)`. When adding a new wrapper variant, verify that delegation arms `Error::NewFeature(e) => e.translate(lang)` are added to both the `En` and `Ko` match branches in the manual impl.
