...
pub fn InfoCard(data: InfoCardData) -> Element {
    rsx! {
        div { class: "flex h-full w-full min-h-0 self-stretch flex-col items-start gap-2.5 max-mobile:gap-2 p-[30px] max-tablet:p-5 max-mobile:p-4 rounded-2xl max-mobile:rounded-xl bg-space-dashboard-card",

            // Header Section
            div { class: "flex w-full items-center justify-between gap-3 max-mobile:gap-2",

                // Left: Icon
                div { class: "flex h-11 w-11 items-center justify-center max-mobile:h-9 max-mobile:w-9 rounded-[10px] bg-space-dashboard-green",
                    icons::ratel::Clock {
                        width: "24",
                        height: "24",
                ...
                div { class: "flex-1 min-w-0 text-right",
                    div { class: "text-[24px] leading-[24px] max-mobile:text-[20px] max-mobile:leading-[22px] font-bold text-text-primary sp-dash-font-inter",
                        "{data.main_value}"
                    ...
                    div { class: "mt-1 text-[15px] leading-[18px] max-mobile:text-[13px] max-mobile:leading-[16px] tracking-[-0.16px] font-semibold text-space-dashboard-muted sp-dash-font-raleway",
                        "{data.main_label}"
                    }
                }
            }
                            ...
                            div { class: "flex items-center justify-between gap-2 text-text-primary",
                                if let Some(text) = boost_label {
                                    div { class: "flex min-w-0 items-center gap-1",
                                        ...
                                        span { class: "truncate text-text-primary text-xs leading-4 font-medium sp-dash-font-inter",
                                            "{text}"
                                        }
                                    }
                                } else {
                                ...
                                span { class: "shrink-0 text-text-primary text-xs leading-4 font-semibold sp-dash-font-inter",
                                    "{item.value}"
                                }
                            }
                        }
