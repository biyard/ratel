...
impl Parse for RouteAttr {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        //   #[get("/path", name: Type, name2: Type2, ...)]   — name + type (type discarded)
        //
...
pub fn server_fn_impl(method: &str, attr: TokenStream, item: TokenStream) -> TokenStream {
    // The macro emits a single HTTP-stub function — the body of the
    // original handler is discarded. Under `fullstack` builds, callers
    // should route `#[get]/#[post]/...` through `dioxus::prelude::*`
    // directly via a cfg-gated re-export in their crate root (see
    // `app/src/lib.rs` in biyard/asset and biyard/ratel), so by-macros
    // is only invoked in non-fullstack (desktop / tauri-web) targets.
