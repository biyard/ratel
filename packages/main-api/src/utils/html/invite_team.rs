...
pub fn invite_team_html(team: Team, url: String) -> String {
    let html = format!(
        r#"<!doctype html>
<html lang="en">
  <head>
      ...
      <tr><td align="center">
        <table role="presentation" width="560" cellspacing="0" cellpadding="0" style="background:#fff;border-radius:14px;padding:24px">
          <tr><td>
            ...
            <h2 style="margin:18px 0 10px; color:#171717; font-size:20px; font-weight:600">You're invited to join {team_name}</h2>
            ...
            <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:8px 0 16px; width:100%;">
              <tr>
                ...
                <td width="8"></td>
                <td valign="middle">
                  <div style="font-weight:700;font-size:16px;color:#171717;margin-bottom:2px;">{team_display_name}</div>
                  <div style="font-weight:600;font-size:12px;color:#8C8C8C;">@{team_name}</div>
                </td>
              </tr>
            </table>

            <p style="color:#262626;font-size:11px">
              If the button doesn’t work, use this link: <a href="{url}">{url}</a>
            </p>
