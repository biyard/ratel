use bdk::prelude::*;
use dto::{
    Follower, Result, UserSuggestionDismissal,
    by_axum::{
        auth::Authorization,
...
pub struct RejectSuggestionResponse {
    pub suggestion: Follower,
}

pub async fn reject_suggestion_handler(
    Extension(auth): Extension<Option<Authorization>>,
    State(pool): State<PgPool>,
    Json(body): Json<RejectSuggestionRequest>,
) -> Result<Json<RejectSuggestionResponse>> {
    let repo = UserSuggestionDismissal::get_repository(pool.clone());
    ...
    Ok(Json(RejectSuggestionResponse { suggestion }))
}
