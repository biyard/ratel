...
use bdk::prelude::*;
use dto::{
    ConnectionInvitationDecline, Follower, Result,
    by_axum::{
        auth::Authorization,
...
pub struct RejectInvitationResponse {
    pub followee_id: i64,
...
pub async fn reject_invitation_handler(
...
) -> Result<Json<RejectInvitationResponse>> {
    let repo = ConnectionInvitationDecline::get_repository(pool.clone());
    ...
    Ok(Json(RejectInvitationResponse {
        followee_id,
        invitation: next,
    }))
}
