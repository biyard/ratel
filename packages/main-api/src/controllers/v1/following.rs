...
impl FollowingController {
    async fn query(
        ...
        let items: Vec<FollowerSummary> = FollowerSummary::query_builder()
            ...
            .await
            .map_err(|e| {
                tracing::error!("Failed to fetch following for user {}: {:?}", user_id, e);
    ...
    async fn follow(
        ...
        let to_be_followed = User::query_builder()
            .id_equals(to_be_followed)
            .query()
            .map(User::from)
            .fetch_one(&self.pool)
            .await
        ...
        let follower = self.repo.insert(
            follower.id,
            follower.nickname,
            follower.profile_url, // follower_profile_image
            None,
            None,
            to_be_followed.id,
            to_be_followed.nickname, // following_profile_image
            to_be_followed.profile_url, // following_description
            None,
            None,
        ).await?;
...
mod tests {
    ...
    async fn test_follow_and_unfollow() {
        ...
        let follower_ids_after: Vec<i64> = query_res_after.items.iter()
            .map(|f| f.follower_id)
            .collect();
        assert!(!follower_ids_after.contains(&user2.id), "User1 should no longer be following user2");
    }
