...
use crate::{
    ...
    models::dynamo_tables::main::vc::{
        issued_credential::IssuedCredential, oauth_access_token::OAuthAccessToken,
    },
        ...
        dto::by_axum::auth::Authorization::Bearer { claims } => {
            ...
            let result = dynamo_client
                ...
                .expression_attribute_values(
                    ":pk",
                    to_attribute_value(&user_partition).map_err(|e| {
                ...
                Ok(output) => {
                    if let Some(items) = output.items {
                        ...
                        Err(dto::Error::Unknown(
                            "No valid access token found for user".to_string(),
                        ))
                    } else {
                        Err(dto::Error::Unknown(
                            "No access tokens found for user".to_string(),
                        ))
                    }
                }
                Err(e) => {
