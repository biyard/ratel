use crate::{
    ...
    models::{
        DeliberationSurveyResponse, User,
...
pub async fn get_deliberation_poll_handler(
    State(AppState { dynamo, .. }): State<AppState>,
    NoApi(user): NoApi<Option<User>>,
    Path(DeliberationPath { space_pk }): Path<DeliberationPath>,
) -> Result<Json<DeliberationSurveyResponse>, Error2> {
    ...
    let mut surveys = metadata.surveys;

    let responses = surveys.clone().responses;

    for response in responses {
        if response.user_pk == user.clone().unwrap_or_default().pk {
            surveys.user_responses.push(response);
        }
    }
