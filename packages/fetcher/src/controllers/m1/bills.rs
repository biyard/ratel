...
impl BillWriterController {
    ...
    pub async fn act_bill(
        ...
        let res = match body {
            ...
            BillWriterAction::FetchBill(param) => ctrl.fetch_bill(param.bill_no).await?,
        };
...
impl BillWriterController {
    ...
    async fn fetch_bill(&self, no: i64) -> Result<BillWriter> {
        tracing::debug!("fetch_bill {}", no);

        let bill = self.cli.get_bill(no).await?;
        tracing::debug!("bill: {:?}", bill);

        let link_url = bill.link_url.clone();
        let parser = HtmlParser::new(&link_url).await?;
        let file_book_id = parser.get_file_book_id()?;
        ...
        // TODO: automatically translation

        // let bill = if let Some(b) = BillWriter::query_builder()
        //     .bill_no_equals(bill.bill_no)
        //     .query()
        //     .map(BillWriter::from)
        ...
        // let repo = BillWriter::get_repository(self.pool.clone());
