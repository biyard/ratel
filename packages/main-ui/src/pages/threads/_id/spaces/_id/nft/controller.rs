...
pub struct Controller {
    ...
    pub current_step: Signal<NftSettingStep>,
}
...
impl Controller {
    pub fn new(lang: Language, feed_id: i64, id: i64) -> std::result::Result<Self, RenderError> {
        ...
        let ctrl = Self {
            ...
            feed_id,
            id,
            current_step: use_signal(|| NftSettingStep::Summary),
            is_edit: use_signal(|| false),
        };

        use_effect(move || {
            nav.replace(Route::NftSummary { feed_id, id });
        });
    ...
    pub fn change_current_step(&mut self, step: NftSettingStep) {
        let lang = self.lang;
