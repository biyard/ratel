        ...
        div {..attributes,
            div { class: "relative w-full h-full inline-block text-left",
                ...
                div {
                    ...
                    class: "absolute right-0 z-10 mt-10 w-56 origin-top-right rounded-[10px] ring-1 ring-primary focus:outline-hidden bg-black overflow-hidden",
                    role: "menu",
                    tabindex: "-1",
                    div { class: "py-1", role: "none",
                        for (i , item) in items.into_iter().enumerate() {
                            div {
                                class: "py-2 text-sm text-gray-700 text-c-wg-50 font-semibold py-15 px-20 flex flex-row w-full justify-between hover:text-white items-center cursor-pointer",
